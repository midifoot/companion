...
class FileEditor(QDialog):
    ...
    def __init__(self, filename, content, parent=None):
        ...
        # Connect vertical bars for scrolling sync
...
class MFKBApp(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("MFKB Companion App v2.0.8")
    ...
    def setup_keymap_tab(self):
        ...
        for i, name in enumerate(midi_notes):
            ...
            lbl.setStyleSheet("color: #2ecc71; font-weight: bold; font-size: 10px;")
            grid_lay.addWidget(lbl, 0, i + 1)
        for row in range(11):
            lbl_oct = QLabel(f"Oct {row-1}")
            lbl_oct.setFixedWidth(40)
        ...
        for sz in splits:
            ...
            for _ in range(sz):
                ...
                row.addWidget(key_lbl); row.addWidget(box)
                vbox.addLayout(row)
                k_ptr += 1
            vbox.addStretch()
        ...
        res_row = QHBoxLayout()
        self.km_res_box = QTextEdit(); self.km_res_box.setObjectName("ResultBox"); self.km_res_box.setFixedHeight(50); self.km_res_box.setReadOnly(True)
        ...
        res_row.addWidget(self.km_res_box); res_row.addWidget(self.btn_copy_km)
    ...
    def setup_chord_tab(self):
        ...
        h1 = QHBoxLayout()
        self.chord_id_edit = QLineEdit("01")
        self.chord_id_edit.setFixedWidth(45)
        self.chord_id_edit.textChanged.connect(self.update_chord_output)
        self.chord_name_edit = QLineEdit("Maj_Chord")
        self.chord_name_edit.setFixedWidth(200)
        self.chord_name_edit.setMaxLength(12)
        self.chord_name_edit.textChanged.connect(self.update_chord_output)
        h1.addWidget(QLabel("C ID:"))
        h1.addWidget(self.chord_id_edit)
        h1.addWidget(QLabel("NAME:"))
        h1.addWidget(self.chord_name_edit)
        h1.addStretch()
        layout.addLayout(h1)
        ...
        grid_lay.addWidget(QLabel("Bass:"), 0, 0)
        for i, val in enumerate([-24, -12]):
            btn = QPushButton(f"{val} (Oct)")
            btn.setObjectName("IntervalBtn")
            btn.setFixedSize(55, 25)
            btn.clicked.connect(lambda checked, v=val: self.on_interval_grid_click(v))
            grid_lay.addWidget(btn, 0, i + 1)
        ...
        for octave in range(3):
            ...
            for st in range(1, 13):
                ...
                label = f"+{val}"
                if st < len(int_names):
                    label += f" ({int_names[st]})"
                btn = QPushButton(label)
                btn.setObjectName("IntervalBtn")
                btn.setFixedSize(65, 25)
                btn.clicked.connect(lambda checked, v=val: self.on_interval_grid_click(v))
                grid_lay.addWidget(btn, octave + 1, st - 1)
        layout.addWidget(grid_frame)
        ...
        for i in range(6):
            vbox = QVBoxLayout()
            lbl = QLabel(f"Voice {i+1}:")
            lbl.setStyleSheet("color: #8b949e; font-size: 9px;")
            box = QLineEdit()
            box.setObjectName("ChordSlot")
            box.setFixedWidth(55)
            box.installEventFilter(self)
            box.textChanged.connect(self.update_chord_output)
            self.chord_inputs.append(box)
            vbox.addWidget(lbl); vbox.addWidget(box)
            slots_hbox.addLayout(vbox)
        slots_hbox.addStretch()
        layout.addLayout(slots_hbox)
        res_row = QHBoxLayout()
        self.chord_res_box = QTextEdit(); self.chord_res_box.setObjectName("ResultBox"); self.chord_res_box.setFixedHeight(50); self.chord_res_box.setReadOnly(True)
        ...
        self.btn_copy_chord.clicked.connect(self.copy_chord_result)
        res_row.addWidget(self.chord_res_box); res_row.addWidget(self.btn_copy_chord)
        layout.addLayout(res_row)
        self.update_active_chord_pointer(0)
        self.update_chord_output()

    # --- TAB 4: PRESET GENERATOR (NEW v2.0.8) ---
    ...
    def setup_preset_tab(self):
        ...
        h1 = QHBoxLayout()
        self.pre_id_edit = QLineEdit("01")
        self.pre_id_edit.setFixedWidth(45)
        self.pre_id_edit.setValidator(QIntValidator(0, 999))
        self.pre_id_edit.textChanged.connect(self.update_preset_output)
        
        self.pre_name_edit = QLineEdit("NewPres")
        self.pre_name_edit.setFixedWidth(120)
        self.pre_name_edit.setMaxLength(7)
        self.pre_name_edit.textChanged.connect(self.update_preset_output)
        
        h1.addWidget(QLabel("REF (P):"))
        h1.addWidget(self.pre_id_edit)
        h1.addWidget(QLabel("NAME:"))
        h1.addWidget(self.pre_name_edit)
        h1.addStretch()
        layout.addLayout(h1)
        
        # STEP 2: LINKS (Drop downs or inputs)
        layout.addWidget(QLabel("STEP 2 : LINK ASSETS", styleSheet="color: #2ecc71; font-weight: bold; font-size: 10px;"))
        grid_frame = QFrame()
        grid_frame.setStyleSheet("background-color: #161b22; border-radius: 4px; border: 1px solid #30363d;")
        grid_link = QGridLayout(grid_frame)
        
        # Root Note
        self.pre_root_combo = QComboBox()
        self.pre_root_combo.addItems(["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"])
        self.pre_root_combo.currentIndexChanged.connect(self.update_preset_output)
        
        # Scales/Maps (Editable Combos)
        ...
        for v in range(-48, 49, 12):
            self.pre_trans.addItem(str(v))
        self.pre_trans.setCurrentText("0")
        
        self.pre_vol.textChanged.connect(self.update_preset_output)
        self.pre_vel.textChanged.connect(self.update_preset_output)
        self.pre_chan.textChanged.connect(self.update_preset_output)
        self.pre_trans.currentIndexChanged.connect(self.update_preset_output)
        
        h_defaults.addWidget(QLabel("Volume:")); h_defaults.addWidget(self.pre_vol)
        h_defaults.addWidget(QLabel("Velocity:")); h_defaults.addWidget(self.pre_vel)
        h_defaults.addWidget(QLabel("Transpose:")); h_defaults.addWidget(self.pre_trans)
        h_defaults.addWidget(QLabel("Channel:")); h_defaults.addWidget(self.pre_chan)
        h_defaults.addStretch()
        layout.addLayout(h_defaults)
        
        # STEP 4: RECAP
        layout.addWidget(QLabel("STEP 4 : PRESET STRING PREVIEW", styleSheet="color: #2ecc71; font-weight: bold; font-size: 10px;"))
        res_row = QHBoxLayout()
        self.pre_res_box = QTextEdit(); self.pre_res_box.setObjectName("ResultBox"); self.pre_res_box.setFixedHeight(50); self.pre_res_box.setReadOnly(True)
        ...
        self.btn_copy_pre.clicked.connect(self.copy_preset_result)
        res_row.addWidget(self.pre_res_box); res_row.addWidget(self.btn_copy_pre)
        layout.addLayout(res_row)
        
        # Refresh combo lists on tab change
        self.tabs.currentChanged.connect(self.on_tab_changed_preset_refresh)
        self.update_preset_output()
    ...
    def refresh_preset_dropdowns(self):
        """Silently scans local FKB files to populate linked assets."""
        self.pre_scan_assets()
        # Save current texts
        bs_txt = self.pre_bs_combo.currentText()
        es_txt = self.pre_es_combo.currentText()
        km_txt = self.pre_km_combo.currentText()
        mm_txt = self.pre_mm_combo.currentText()
        
        self.pre_bs_combo.clear(); self.pre_bs_combo.addItems(sorted(list(self.asset_db["SCALES"])))
        self.pre_es_combo.clear(); self.pre_es_combo.addItems(sorted(list(self.asset_db["SCALES"])))
        self.pre_km_combo.clear(); self.pre_km_combo.addItems(sorted(list(self.asset_db["KEYMAPS"])))
        self.pre_mm_combo.clear(); self.pre_mm_combo.addItems(sorted(list(self.asset_db["MIDIMAPS"])))
    ...
    def update_preset_output(self):
        p_id = self.pre_id_edit.text().zfill(3)
        p_name = self.pre_name_edit.text()
        root = self.pre_root_combo.currentText()
        bs = self.pre_bs_combo.currentText()
        es = self.pre_es_combo.currentText()
        km = self.pre_km_combo.currentText()
        mm = self.pre_mm_combo.currentText()
        vol = self.pre_vol.text()
        vel = self.pre_vel.text()
        tr = self.pre_trans.currentText()
        ch = self.pre_chan.text()
    ...
    def copy_preset_result(self):
        ...
        self.btn_copy_pre.setText("COPIED!")
        QTimer.singleShot(1500, lambda: self.btn_copy_pre.setText("COPY"))
    ...
    def copy_chord_result(self):
        ...
        self.btn_copy_chord.setText("COPIED!")
        QTimer.singleShot(1500, lambda: self.btn_copy_chord.setText("COPY"))
    ...
    def setup_sd_tab(self):
        ...
        h_row.addWidget(QLabel("Caution: Edit only LOCAL FILES!", styleSheet="color: #FF0000; font-weight: bold; font-size: 10px;"))
        h_row.addStretch()
        ...
        self.btn_validate = QPushButton("CHECK INTEGRITY"); self.btn_validate.setObjectName("SD_Btn"); self.btn_validate.setFixedHeight(30)
        self.btn_validate.clicked.connect(self.run_integrity_check); left_box.addWidget(self.btn_validate)
        ...
        mid_box.addWidget(self.btn_to_sd); mid_box.addSpacing(10); mid_box.addWidget(self.btn_edit_sd); mid_box.addSpacing(10); mid_box.addWidget(self.btn_del); mid_box.addSpacing(10); mid_box.addWidget(self.btn_from_sd)
    ...
    def set_busy(self, busy):
        ...
        self.btn_del.setEnabled(st); self.btn_refresh.setEnabled(st); self.btn_validate.setEnabled(st)
